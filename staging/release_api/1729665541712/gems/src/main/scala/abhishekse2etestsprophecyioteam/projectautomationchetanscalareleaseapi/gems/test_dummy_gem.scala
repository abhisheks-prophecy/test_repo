package abhishekse2etestsprophecyioteam.projectautomationchetanscalareleaseapi.gems
...
class Reformat extends ComponentSpec {
  ...
  val name: String = "test_dummy_gem"
  ...
  def validate(component: Component)(implicit context: WorkflowContext): Diagnostics = {
    ...
    ).map(_.setDiagnosticFix(CopilotDiagnosticFix.copilotExpressionFix))
    diagnostics.toList
  }
